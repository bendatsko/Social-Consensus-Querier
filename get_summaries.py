...
def summarize_comments(client):
    ...
    for article_id, ny_times_title in articles:
        ...
        for reddit_post in reddit_posts:
            ...
            relevant_texts = [reddit_title] + [comment for comment in comments if
                                               comment and is_relevant(comment, keywords)]
        ...
        if full_text:
            document = [full_text]
            poller = client.begin_analyze_actions(document, actions=[ExtractiveSummaryAction(max_sentence_count=1)])
            document_results = poller.result()

            for result in document_results:
                for summary_result in result:
                    if summary_result.is_error:
                        print(f"Error: {summary_result.code} - {summary_result.message}")
                    else:
                        summary = "".join(sentence.text for sentence in summary_result.sentences)
                        cursor.execute('INSERT INTO article_summaries (article_id, summary) VALUES (?, ?)',
                                       (article_id, summary))
